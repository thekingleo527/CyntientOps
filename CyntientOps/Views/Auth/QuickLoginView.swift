...
struct QuickLoginView: View {
        ...
        NavigationView {
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
                    Button("Health") { Task { await dumpDBHealth() } }
                }
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button("Reload") { Task { await loadUsers() } }
                }
            }
            ...
            .alert("DB Health", isPresented: $showingHealth) {
                Button("OK") { showingHealth = false }
            } message: { Text(healthText) }
        }
    }
    ...
    private func dumpDBHealth() async {
        isLoading = true
        defer { isLoading = false }
        let text = await db.dumpHealth()
