...
public final class RouteManager: ObservableObject {
    ...
    public static let shared: RouteManager = {
        ...
        return RouteManager(database: db)
    }()
    ...
    private func setupRoutes() {
        ...
        // TODO: Add when route files are available
    ...
    /// Get today's route items for hero cards
    public func today(for workerId: String, date: Date) -> [RouteItem] {
        guard let route = getCurrentRoute(for: workerId) else { return [] }
        
        return route.sequences.map { sequence in
            RouteItem(
                id: sequence.id,
                buildingName: sequence.buildingName,
                buildingId: sequence.buildingId,
                time: DateFormatter.timeOnly.string(from: sequence.arrivalTime),
                icon: iconForSequence(sequence),
                isActive: isSequenceActive(sequence),
    ...
    public func nextUp(for workerId: String, from date: Date) -> RouteItem? {
        let upcomingSequences = getUpcomingSequences(for: workerId, limit: 1)
        guard let nextSequence = upcomingSequences.first else { return nil }
        
        return RouteItem(
            id: nextSequence.id,
            buildingName: nextSequence.buildingName,
            buildingId: nextSequence.buildingId,
            time: DateFormatter.timeOnly.string(from: nextSequence.arrivalTime),
            icon: iconForSequence(nextSequence),
            isActive: false, // Next up is not active yet
            dayOfWeek: nil
        )
    }
    ...
    private func isSequenceActive(_ sequence: RouteSequence) -> Bool {
        let now = Date()
        let endTime = sequence.arrivalTime.addingTimeInterval(sequence.estimatedDuration)
        return sequence.arrivalTime <= now && now <= endTime
    }
    
    private func iconForSequence(_ sequence: RouteSequence) -> String {
        switch sequence.sequenceType {
        case .stairwellCleaning:
            return "stairs"
