...
public class WorkerDashboardViewModel: ObservableObject {
    ...
    // MARK: - Site Departure (Multi‑Site)
    @Published public private(set) var pendingDepartures: [BuildingSummary] = []

    // MARK: - Photo Prompt State
    public struct PhotoPrompt: Identifiable {
        public let id = UUID()
        public let task: TaskItem
        public let category: CoreTypes.CyntientOpsPhotoCategory
        public let isDSNYSetOut: Bool
    }
    @Published public var pendingPhotoPrompt: PhotoPrompt?
    ...
    public func refreshData() async {
        ...
        await performSync { [weak self] in
            ...
            // Refresh pending departures after data loads
            await self.refreshPendingDepartures()
    ...
    private func loadAssignedBuildings() async {
        ...
        do {
            ...
            // Calculate optimal map region from preferred map dataset
            updateMapRegion(with: preferredMapBuildings())
    ...
    /// Compute preferred buildings for map display (fallback ensures non-empty set)
    private func preferredMapBuildings() -> [BuildingSummary] {
        ...
        if !portfolioBuildings.isEmpty {
            // Convert portfolio (NamedCoordinate) into BuildingSummary coverage entries
            return portfolioBuildings.map { b in
                BuildingSummary(
                    id: b.id,
                    name: b.name,
                    address: b.address,
    ...
    /// Update map region to show provided buildings optimally
    private func updateMapRegion(with buildings: [BuildingSummary]) {
        ...
        if buildings.count == 1 {
            ...
            let span = MKCoordinateSpan(
                latitudeDelta: max(0.008, (maxLat - minLat) * 1.3),
                longitudeDelta: max(0.008, (maxLon - minLon) * 1.3)
            )
            mapRegion = MKCoordinateRegion(center: center, span: span)
        }
        print("✅ Updated map region for \(buildings.count) buildings (preferred set)")
    }
    ...
    /// Determine which buildings still need a departure log today.
    public func refreshPendingDepartures() async {
        guard let workerId = currentWorkerId else { pendingDepartures = []; return }
        // Determine sites VISITED today using time_clock_entries (authoritative) with fallbacks.
        ...
        do {
            let rows = try await container.database.query(
                """
                SELECT DISTINCT buildingId AS bid
                FROM time_clock_entries
                WHERE workerId = ?
                  AND clockInTime >= ? AND clockInTime < ?
                """,
                [workerId, ISO8601DateFormatter().string(from: dayStart), ISO8601DateFormatter().string(from: dayEnd)]
            )
            visitedIds.formUnion(rows.compactMap { $0["bid"] as? String })
        } catch {
            print("⚠️ Visited query (time_clock_entries) failed: \(error)")
        }
        // Fallbacks: task completions and departure logs also imply visit
        do {
            let rows = try await container.database.query(
                """
                SELECT DISTINCT building_id AS bid
                FROM task_completions
                WHERE worker_id = ?
                  AND completion_time >= ? AND completion_time < ?
                """,
                [workerId, ISO8601DateFormatter().string(from: dayStart), ISO8601DateFormatter().string(from: dayEnd)]
            )
            visitedIds.formUnion(rows.compactMap { $0["bid"] as? String })
        ...
        // Compute pending set by removing those already departed
        var pending: [BuildingSummary] = []
        for bid in visitedIds {
            do {
                let alreadyDeparted = try await SiteLogService.shared.hasWorkerDepartedToday(workerId: workerId, buildingId: bid)
                if alreadyDeparted { continue }
                if let b = assignedBuildings.first(where: { $0.id == bid }) ?? allBuildings.first(where: { $0.id == bid }) {
                    pending.append(b)
                }
            } catch {
                if let b = assignedBuildings.first(where: { $0.id == bid }) ?? allBuildings.first(where: { $0.id == bid }) {
        ...
        // Only enforce multi‑site flow for specific workers; others keep current behavior
        if let wid = Int(workerId), [2,4,6,7].contains(wid) {
            pendingDepartures = pending
        } else {
            // For Greg (1) and Mercedes (5), keep single‑site or simple dashboard
            pendingDepartures = pending.prefix(1).map { $0 }
        }
    }

    /// Complete a site departure for a specific building.
    public func completeDeparture(for buildingId: String, method: DepartureMethod = .normal) async {
        guard let workerId = currentWorkerId else { return }
        do {
            // Compile departure checklist for this site
            let checklist = try await container.tasks.getDepartureChecklistItems(for: workerId, buildingId: buildingId)
            let isCompliant = checklist.incompleteTasks.isEmpty
            // Use current location if available via ClockInService
            let status = container.clockIn.getClockInStatus(for: workerId)
            let location = status?.location
            // Create the departure log (broadcasts via DashboardSync)
            try await SiteLogService.shared.createDepartureLog(
                workerId: workerId,
            ...
            await refreshPendingDepartures()
        } catch {
            print("❌ Failed to complete site departure for building \(buildingId): \(error)")
        }
    }

    /// End-of-day clock out for the current worker (after all departures).
    public func endOfDayClockOut() async {
        guard let workerId = currentWorkerId else { return }
        do {
            try await container.clockIn.clockOut(workerId: workerId)
        } catch {
            print("❌ Failed to clock out at end of day: \(error)")
        }
    }
    ...
    private func updateHeroTileProperties() async {
        ...
        // Update buildings for map (prefer assigned → all → portfolio)
        let mapBuildings = preferredMapBuildings()
        buildingsForMap = mapBuildings.map { building in
            let status: BuildingPin.BuildingStatus
            if currentBuilding?.id == building.id {
                status = .current
            } else if assignedBuildings.contains(where: { $0.id == building.id }) {
                status = .assigned
            } else {
                status = .available
            }
        ...
        updateMapRegion(with: mapBuildings)
    }
    ...
    public func toggleTaskCompletion(_ taskId: String) async {
        ...
        // Persist status via TaskService
        let status: CoreTypes.TaskStatus = todaysTasks[index].isCompleted ? .completed : .pending
        do {
            try await container.tasks.updateTaskStatus(taskId, status: status)
        } catch {
            print("❌ Failed to update task status: \(error)")
        }

        // Photo prompt when marking completed for specific tasks/workers
        if status == .completed,
           let prompt = determinePhotoPrompt(for: updatedTask) {
            pendingPhotoPrompt = prompt
        }
    }

    /// Determine if a completed task should prompt for photo evidence and which category to use
    private func determinePhotoPrompt(for task: TaskItem) -> PhotoPrompt? {
        guard let workerId = currentWorkerId else { return nil }
        let wid = Int(workerId) ?? -1
        let title = task.title.lowercased()

        // Never prompt for glass cleaning
        if title.contains("glass") || title.contains("window") { return nil }

        // Target tasks
        let isTrashArea = title.contains("trash area") || title.contains("trash")
        let isGarbageSetOut = title.contains("set out") || title.contains("set-out") || title.contains("dsny")
        let isPosterRemoval = title.contains("poster") || title.contains("sticker")
        let isMiscProject = title.contains("project") || title.contains("misc") || title.contains("repair")
        let isSidewalkHose = title.contains("hose") || title.contains("sidewalk")

        // Kevin (4), Greg (1), Angel (7): before/after style → we capture After at completion
        if [1,4,7].contains(wid) && (isTrashArea || isGarbageSetOut || isPosterRemoval || isMiscProject) {
            let cat: CoreTypes.CyntientOpsPhotoCategory = isGarbageSetOut ? .compliance : .afterWork
            return PhotoPrompt(task: task, category: cat, isDSNYSetOut: isGarbageSetOut)
        }

        // Luis (6): end-of-day pics for garbage set out, possibly sidewalk hose — prompt on completion of those specific tasks
        if wid == 6 && (isGarbageSetOut || isSidewalkHose) {
            let cat: CoreTypes.CyntientOpsPhotoCategory = isGarbageSetOut ? .compliance : .afterWork
            return PhotoPrompt(task: task, category: cat, isDSNYSetOut: isGarbageSetOut)
        }
